...
def _repo_root() -> Path:
    # .../projects/pca_vs_ttd/experiments/tradeoff-sweep-v2/run_sweep.py
...
def _plot_tradeoff(
    ...
    def _series(method: str, *, predicate=None) -> tuple[list[float], list[float], list[float]]:
        xs: list[float] = []
        ys_rel: list[float] = []
        ys_psnr: list[float] = []
        for r in rows:
    ...
    ttd_equal_x, ttd_equal_rel, ttd_equal_psnr = _series(
        "ttd", predicate=lambda r: int(r.get("r1") or 0) == int(r.get("r2") or -1)
    )
    ttd_unequal_x, ttd_unequal_rel, ttd_unequal_psnr = _series(
        "ttd", predicate=lambda r: int(r.get("r1") or 0) != int(r.get("r2") or 0)
    )

    fig, ax = plt.subplots(figsize=(7, 5))
    ax.plot(pca_x, pca_rel, "o-", label="PCA")
    ax.plot(ttd_equal_x, ttd_equal_rel, "o-", label="TTD (TT-SVD), equal ranks")
    ax.plot(ttd_unequal_x, ttd_unequal_rel, "x", label="TTD (TT-SVD), unequal ranks")
    ax.set_xscale("log")
    ax.set_yscale("log")
    ...
    ax.plot(ttd_equal_x, ttd_equal_psnr, "o-", label="TTD (TT-SVD), equal ranks")
    ax.plot(ttd_unequal_x, ttd_unequal_psnr, "x", label="TTD (TT-SVD), unequal ranks")
    ax.set_xscale("log")
    ax.set_xlabel("compression_ratio (log)")
...
def _parse_rank_pairs(text: str) -> list[tuple[int, int]]:
    pairs: list[tuple[int, int]] = []
    for part in [p.strip() for p in text.split(";") if p.strip()]:
        items = [x.strip() for x in part.split(",") if x.strip()]
        if len(items) != 2:
            raise ValueError(f"Invalid rank pair '{part}'; expected 'r1,r2'.")
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Run PCA+TTD trade-off sweep (including unequal TT ranks) and write summary CSV + plots."
    )
    ap.add_argument(
        "--data",
        type=Path,
    ...
    ap.add_argument(
        "--out-dir",
        type=Path,
        default=Path("projects/pca_vs_ttd/experiments/tradeoff-sweep-v2/results"),
    ...
    ap.add_argument(
        "--ttd-rank-pairs",
        type=str,
        default=";".join(f"{r1},{r2}" for (r1, r2) in DEFAULT_TTD_RANK_PAIRS),
        help="Semicolon-separated TT rank pairs (e.g. '8,8;16,32').",
    )
    ...
    ttd_rank_pairs = _parse_rank_pairs(args.ttd_rank_pairs)
    ...
    for (r1, r2) in ttd_rank_pairs:
        out = out_dir / f"ttd_r{r1}_{r2}.json"
        cmd = [
            sys.executable,
            str(ttd_script),
            "--data",
