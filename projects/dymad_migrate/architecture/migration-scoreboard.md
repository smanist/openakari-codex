...
- training now has an adopted seam (`TrainerRun` + `PhasePipeline`) with linear workflow parity and one non-linear `NODE` workflow instrumented through `TrainerRun`; compatibility adapters remain temporary
