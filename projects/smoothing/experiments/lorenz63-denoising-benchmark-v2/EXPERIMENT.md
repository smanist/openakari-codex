...
- 2026-04-28 confirmatory execution completed the durable artifact bundle under `modules/smoothing/artifacts/lorenz63-denoising-benchmark-v2/confirmatory/` using the six pilot-selected finalists recorded in `projects/smoothing/experiments/lorenz63-denoising-benchmark-v2/confirmatory_settings.json`.
- A timed benchmark showed the full confirmatory command completes in about `14` seconds, so this stage also stayed below the repo's `2` minute detached-run threshold and was executed inline rather than through `infra/experiment-runner/run.py --detach`.
- The confirmatory bundle writes the dataset snapshot, `metrics_raw.csv`, `summary_by_setting.csv`, `best_by_noise.csv`, `robust_settings.csv`, `family_comparison.csv`, `run_manifest.json`, `output.log`, and four comparison plots including `derivative_rmse_vs_noise.png`.
- The overall experiment remains `in_progress`: pilot execution, pilot analysis, and confirmatory execution are complete, but confirmatory analysis is still pending.
...
- `/usr/bin/time -p python modules/smoothing/run_denoising_sweep_v2.py --out-dir /tmp/lorenz63-v2-confirmatory-benchmark-$$ --stage confirmatory --confirmatory-settings-json projects/smoothing/experiments/lorenz63-denoising-benchmark-v2/confirmatory_settings.json --overwrite >/tmp/lorenz63-v2-confirmatory-benchmark-$$.stdout`
  Output: `real 14.03`, `user 13.74`, `sys 0.14`, establishing that the full confirmatory stage stays below the repo's detached-run threshold.
...
- `python - <<'PY' ... PY` counting rows in `modules/smoothing/artifacts/lorenz63-denoising-benchmark-v2/confirmatory/{metrics_raw.csv,summary_by_setting.csv,best_by_noise.csv,robust_settings.csv,family_comparison.csv}` and reading `run_manifest.json`
  Output: `metrics_raw.csv 896`, `summary_by_setting.csv 56`, `best_by_noise.csv 20`, `robust_settings.csv 16`, `family_comparison.csv 20`, `manifest_counts {'n_best_rows': 20, 'n_family_comparison_rows': 20, 'n_robust_rows': 16, 'n_rows_expected': 896, 'n_rows_written': 896, 'n_samples': 64, 'n_settings': 14, 'n_summary_rows': 56}`, `manifest_plots ['modules/smoothing/artifacts/lorenz63-denoising-benchmark-v2/confirmatory/plots/rmse_vs_noise.png', 'modules/smoothing/artifacts/lorenz63-denoising-benchmark-v2/confirmatory/plots/relative_rmse_vs_noise.png', 'modules/smoothing/artifacts/lorenz63-denoising-benchmark-v2/confirmatory/plots/denoising_gain_vs_noise.png', 'modules/smoothing/artifacts/lorenz63-denoising-benchmark-v2/confirmatory/plots/derivative_rmse_vs_noise.png']`, and repo-relative dataset paths under `modules/smoothing/artifacts/lorenz63-denoising-benchmark-v2/confirmatory/dataset/`.
